...
static uint8_t handler_count[EV_MAX];	//存放每个事件EV(数量EV_MAX)的已有"订阅者"数量
...
// 订阅带int参数的函数
void EventBus_SubscribeInt(EventType ev, void (*handler)(int), int param) {
    if (ev < EV_MAX && handler_count[ev] < MAX_SUBSCRIBERS)
	{
...
#define EVENT_QUEUE_SIZE 4  				// 最大事件数量

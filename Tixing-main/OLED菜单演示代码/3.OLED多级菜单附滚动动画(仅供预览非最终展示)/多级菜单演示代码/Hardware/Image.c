...
/**
  * @brief  创建新队列作为子层菜单,绑定当前中栏的图像结点所含子链表,显示区块再指向新队列
  * @param  无
  * @retval 无
  */
...
void MenuBack(void)//返回上级菜单,目前仅2级菜单,无命令
{
	...
	Queue *DestroyQueue = pCurrentQueue;//指针指向要销毁队列
	pCurrentQueue = (struct Queue*)pCurrentQueue->Parent;//显示区块指向其父队列,即显示父菜单
	
	Queue_Destroy(DestroyQueue);	//销毁队列释放内存
...
void ShowMenu(u8 Update_Bit)
{
	...
	{		
		...
		//OLED_ClearBufArea(p->block[0],p->block[1],p->block[2],p->block[3]);// 把旧图擦掉,根据OLED_ShowImage函数是否先擦再写而调用
		
		if(p->sequence!=2 && p->element->id =='B' )//当p不是中间框时,若此框存放的菜单元素(LinkNode)不是(默认尺寸24*32的)B结点时,把当前元素尺寸强转为24*32;
		{//把右侧元素图片大小强转为24*32
			scale_1bit_1p5x(p->element->Image,
					 36,48,		//图片缩放  //36,48
					 ImageTemp,
					 24,  32);
			/* ---------------- 写入缓存数组---------------- */
		...
		//更改图像尺寸大小
		else if(p->sequence==2 && p->element->id !='B')
		{
...
void MenuDown(uint8_t Step)
{
	...
	{	
		//if(Line-Step>0) OLED_ClearBufArea(Line,0,8,128);
...
/**
  * @brief  将任意尺寸缓存数组knn算法通过knn算法拉伸为任意尺寸,除法改移位提快算力
  * @param  源图像数组,源高,源宽,目标图像数组,目标高,目标宽
  * @retval 缓冲区dst指针
  */
uint8_t* scale_1bit_1p5x(const uint8_t *src,
